...
numVal=32*4;
val = [new_list(1:numVal,:) ; new_list(size(new_list,1)/2:size(new_list,1)/2+numVal-1,:)];%:200,:)];%end-(batch_size*15-1):end);
val_label = [label(1:numVal,:) ; label(size(new_list,1)/2:size(new_list,1)/2+numVal-1,:)];%label(end-(batch_size*15-1):end,:);
new_list = [new_list(numVal+1:size(new_list,1)/2-1,:) ; new_list(size(new_list,1)/2+numVal:end,:)];%end-batch_size*15);
label = [label(numVal+1:size(new_list,1)/2-1,:) ; label(size(new_list,1)/2+numVal:end,:)];%end-batch_size*15);

% label = label(1:end-batch_size*15,:);
count_badval=0;
n = length(new_list);
...
for iter = 1 : param.epochs
    ...
    if mod(iter,5) == 0
        ...
        val_err = get_cross_entropy_all(model, val, val_label, num_layer);        
        fprintf('>> iter : %d, val cross_entropy is : %f\n', iter, val_err);
                
        if exist('last_val','var') && val_err > last_val
            count_badval=count_badval+1;
        elseif exist('last_val','var')
            count_badval=0;
        end
        
        if count_badval>=5
            disp('}}} break because val error going up');
            break;
        end
