...
global kConv_backward kConv_forward kConv_weight;
...
for iter = 1 : param.epochs
    ...
    for b = 1 : batch_num
        ...
        hidden_presigmoid = myConvolve(kConv_forward, batch, all_ones, stride, 'forward');
        the_filter = (hidden_presigmoid == 0);
        %% positive phase : use data-dependent expectation
        % first propagate upwards
        hidden_presigmoid = myConvolve(kConv_forward, batch, model.layers{l}.w, stride, 'forward');
        ...
        for k = 1 : param.kPCD
            ...
            % PROPUP 
            neg_hidden_presigmoid = myConvolve(kConv_forward, visible_sample, model.layers{l}.w, stride, 'forward');
    ...
    if mod(iter,5) == 0
        ...
        train_err = get_cross_entropy(model, new_list, l);
