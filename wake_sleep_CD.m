...
% This the version that I changed, tuning the weight without untying the
...
display('generatively fine-tuning the model........');

global kConv_backward  kConv_backward_c kConv_forward kConv_forward_c kConv_weight kConv_weight_c;
...
err = get_cross_entropy_all(model, new_list, label);
...
for iter = 1 : param.epochs
    ...
    for b = 1 : batch_num
		...
		for l = 2 : num_layer
            if l == 2
	            presigmoid = myConvolve(kConv_forward, wake_activations{l-1}, model.layers{l}.w, model.layers{l}.stride, 'forward');
		...
		for l = 1 : num_layer - 2
			if l+1 == 2
				presigmoid = myConvolve(kConv_forward, sleep_activations{l}, model.layers{l+1}.w, model.layers{l+1}.stride, 'forward');
