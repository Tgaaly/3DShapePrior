function [prediction_o, prediction_next, NBV_v, RC_v, RAND_v, FUR_v]= NBV_onestep(model, filename, volume_size, pad_len, angle_inc)
...
data_size = volume_size + 2 * pad_len;
...
for viewpoint = 1 : 360/angle_inc
    ...
    % decide the NBV
    [ignore, NBV_v] = min(NBV_entropy);
    fprintf('choose the %d view\n', NBV_v);
    % decide the RC
    [ignore, RC_v] = max(RC_freespace);
    %decide the FUR
    [ignore, FUR_v] = max(FUR_dis);
    % decide random
    RAND_v = randi(num_v);
end
