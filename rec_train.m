function [predicted_label_tr] = rec_train(model,debug)
% Given the full 3D shape, recognition train for a generative model.
% After the model is (generatively) trained, this function train the
% recognition ability(discriminative) of the model using a seperate train
% data.
...
train_list = read_data_list(model.data_path, model.classnames, ...
...
for i = 1 : model.classes
    cnt = length(train_list{i});
    label(now:now+cnt-1,1) = i;
    new_list(now:now+cnt-1,1) = train_list{i};
    now = now + cnt;
end
n = length(label);
...
predicted_label_tr = zeros(n, model.classes); % prediction results.
for b = 1 : batch_num
    ...
    for c = 1 : model.classes
        ...
        try_label(:, c) = 1;
        predicted_label_tr((b-1) * batch_size+1: batch_end, c) = free_energy(model, [try_label, batch_data], num_layer);
    end
    
end

predicted_label_tr = bsxfun(@rdivide, predicted_label_tr, sum(predicted_label_tr, 2));
[~, predicted_label_tr] = max(predicted_label_tr, [], 2);
[predicted_label_tr label]
acc = sum(predicted_label_tr == label) / n;
fprintf('acc is : %f\n', acc * 100);
